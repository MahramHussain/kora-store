...
export function TrendingCarousel({ products }: { products: any[] }) {
  ...
  return (
    <section className="px-6 mb-16">
      ...
      <div 
        className="relative w-full h-[520px] md:h-[500px] bg-white rounded-3xl border border-slate-200 overflow-hidden group shadow-[0_10px_40px_-15px_rgba(0,0,0,0.1)] hover:shadow-[0_10px_50px_-10px_rgba(249,115,22,0.2)] transition-shadow focus:outline-none focus:ring-1 focus:ring-orange-500/50"
      ...
      >
        ...
        >
          {products.map((item) => (
            <div key={item.id} className="w-full h-full shrink-0 flex flex-col-reverse md:flex-row items-center justify-between gap-4 md:gap-8 px-6 py-8 md:px-16 md:py-12">
              <div className="flex-1 text-center md:text-left flex flex-col justify-center h-full">
                ...
                <h3 className="text-2xl md:text-6xl font-black text-slate-900 mb-2 md:mb-4 leading-tight tracking-tighter drop-shadow-sm line-clamp-2">
                  {item.name}
                </h3>
                
                <p className="text-xl md:text-3xl font-bold text-slate-700 mb-4 md:mb-8 font-sans">
                  {CURRENCY}{String(item.price).replace(CURRENCY.trim(), '').replace('$', '').trim()}
                </p>
                
                <div>
                  <Link href={`/shop/${item.id}`} className="inline-block bg-slate-900 text-white hover:bg-orange-500 hover:text-white px-6 py-2.5 md:px-10 md:py-4 rounded-full font-black text-xs md:text-sm uppercase tracking-wider transition-all duration-300 shadow-[0_5px_15px_rgba(0,0,0,0.1)] hover:shadow-[0_0_20px_rgba(249,115,22,0.4)] hover:scale-105">
                    Secure Now
                  </Link>
                </div>
              </div>
              ...
              <div className="flex-1 flex justify-center items-center relative h-full w-full">
                <div className="absolute w-48 h-48 md:w-80 md:h-80 bg-orange-100/50 rounded-full blur-3xl group-hover:bg-orange-200/80 transition-colors duration-700"></div>
                <img 
                  src={item.images?.[0] || item.image} 
                  alt={item.name} 
                  className="relative z-20 w-44 h-44 md:w-[400px] md:h-[400px] object-contain drop-shadow-xl group-hover:scale-110 group-hover:-rotate-3 transition-transform duration-700 ease-out" 
                />
              </div>
            </div>
          ))}
