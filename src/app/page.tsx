...
export default async function Home() {
  ...
  return (
    <main className="min-h-screen bg-white text-slate-900 font-sans selection:bg-kora selection:text-white pb-20">
      ...
      <nav className="flex items-center gap-8 px-6 py-4 overflow-x-auto text-sm font-pixel tracking-wider uppercase whitespace-nowrap border-b border-slate-200 bg-slate-50 scrollbar-hide shadow-sm">
        ...
        <Link href="/shop" className="text-slate-600 hover:text-kora transition-colors">Shop</Link>
        <Link href="/shop?tag=Latest" className="text-slate-600 hover:text-emerald-600 transition-colors">Latest</Link>
        <Link href="/shop?tag=Trending" className="text-slate-600 hover:text-rose-600 transition-colors">Trending</Link>
      </nav>

      {/* 4. PROMO BANNER */}
      <div className="bg-purple-100 border-b border-purple-200 py-2 text-center text-sm font-medium font-pixel text-purple-900">
        🎉 <span className="text-purple-700">20% Off All Elite Boots + Free Shipping.</span> Use code <span className="font-bold text-kora">KORA20</span>
      </div>
      ...
      <section className="px-6 py-12">
        <div className="flex flex-col md:flex-row items-start md:items-center gap-8 mb-8">
          <div className="flex flex-col shrink-0">
            <h2 className="text-2xl font-pixel uppercase tracking-wider">Shop Your Team</h2>
            {/* Swapped the <button> for a <Link> that points to The Vault */}
            <Link href="/shop" className="mt-4 text-xs font-pixel border border-slate-300 rounded-full py-2 px-4 hover:bg-slate-100 text-slate-600 transition-colors w-max text-center">
              See All
            </Link>
          </div>
            ...
            <div className="flex gap-4 overflow-x-auto py-4 scrollbar-hide flex-1 w-full">
              {[
                ...
                { 
                  ...
                  glow: 'group-hover:shadow-[0_0_15px_rgba(0,0,0,0.2)] group-hover:border-slate-300',
                ...
                <Link href={`/shop?team=${team.name}`} key={i} className="flex flex-col items-center gap-3 shrink-0 cursor-pointer group px-2">
                  ...
                  {/* Team Name */}
                  <span className={`text-[10px] font-pixel text-slate-500 text-center max-w-[80px] transition-colors duration-500 uppercase ${team.text}`}>
                    {team.name}
                  </span>
        ...
        <section className="px-6 mb-16">
          <div className="flex justify-between items-end mb-6">
            <h2 className="text-2xl font-pixel uppercase flex items-center gap-4">
          ...
          {latestProduct && (
            <div className="relative w-full h-[400px] rounded-2xl overflow-hidden group border border-slate-200 hover:border-kora/50 transition-colors shadow-2xl bg-white">
              <div className="absolute inset-0 bg-gradient-to-r from-slate-50 via-white/80 to-transparent z-10"></div>
              <div className="absolute right-0 top-0 w-1/2 h-full bg-purple-50/50 group-hover:bg-purple-100/50 transition-colors z-0"></div>
              
              <div className="absolute top-1/2 -translate-y-1/2 left-6 md:left-12 z-20 max-w-lg">
                <p className="text-kora font-pixel tracking-widest text-xs md:text-sm uppercase mb-3">Exclusive Drop</p>
                <h3 className="text-4xl md:text-6xl font-black text-slate-900 mb-4 leading-tight tracking-tighter uppercase break-words font-sans">
                  {latestProduct.name}
                </h3>
                <p className="text-slate-600 mb-8 max-w-sm text-sm md:text-base line-clamp-3 font-sans">
                ...
                <Link href={`/shop/${latestProduct.id}`} className="inline-block bg-kora text-white px-8 py-3 rounded-full font-pixel text-xs hover:scale-105 transition-transform shadow-[0_0_20px_rgba(107,0,255,0.4)]">
                  Shop The Drop
                </Link>
              </div>

              <div className="absolute right-[-10%] md:right-10 top-1/2 -translate-y-1/2 z-10 opacity-50 group-hover:opacity-100 group-hover:scale-105 transition-all duration-700 w-[300px] md:w-[500px] h-[300px] md:h-[500px] bg-kora/10 rounded-full blur-[100px]"></div>
              
              <img 
                src={latestProduct.images[0]} 
                alt={latestProduct.name}
                className="absolute right-0 md:right-20 top-1/2 -translate-y-1/2 z-20 object-contain w-64 h-64 md:w-[400px] md:h-[400px] group-hover:scale-110 group-hover:-rotate-3 transition-transform duration-700 pointer-events-none drop-shadow-2xl"
              />
            </div>
          )}
        </section>
      ...
      {/* 8. RECENTLY UNLOCKED (Horizontal Slider) */}
        <section className="px-6 mb-24 max-w-7xl mx-auto w-full">
          ...
          <div className="flex justify-between items-end mb-6 border-b border-slate-200 pb-4">
            <div>
              <h2 className="text-2xl font-pixel uppercase tracking-wider flex items-center gap-2">
                Recently Unlocked
              </h2>
              <p className="text-sm text-slate-500 mt-2 font-pixel">The newest gear added to The Vault.</p>
            </div>
            {/* The See All Button */}
            <Link 
              href="/shop?tag=Latest" 
              className="text-xs font-pixel text-kora hover:text-purple-700 transition-colors uppercase tracking-wider flex items-center gap-2 group"
            >
              See All 
