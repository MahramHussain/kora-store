...
const TEAMS = ["All Teams", "Arsenal", "Man City", "Real Madrid", "Barcelona", "Bayern", "Juventus", "AC Milan", "PSG", "Tottenham", "Al Nassr", "Inter Miami"];
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {filteredProducts.map((product) => (
            <Link href={`/shop/${product.id}`} key={product.id} className="group relative bg-[#0a0514] rounded-2xl border border-white/10 hover:border-purple-500/50 transition-colors shadow-lg overflow-hidden flex flex-col h-[380px]">
              <div className="relative flex-1 bg-white/5 flex items-center justify-center p-8 overflow-hidden">
                ...
                <img 
                  ...
                  alt={product.name} 
                  className="w-32 h-32 object-contain group-hover:scale-110 group-hover:-rotate-3 transition-all duration-500 opacity-50" 
                />
              </div>
              ...
              <div className="p-5 border-t border-white/5 relative z-20 bg-[#0a0514]">
                <div className="flex justify-between items-start mb-4">
                  ...
                  <span className="text-lg font-black text-white ml-2">${product.price}</span>
                </div>
